...
public class Board {
    ...
    public void addRandomTile() {
        ...
        BoardSpot t = openSpaces.get(random);
        double generate = StdRandom.uniform(0.0, 1.0);
        if(generate < 0.1)
        {
    ...
    public void swipeLeft() {
        for(int i=0; i<gameBoard.length; i++)
        {
            int[] tempArr = new int[gameBoard.length];
            int count = 0;
            for(int j=0; j<gameBoard[0].length; j++)
            {
                if(gameBoard[i][j] != 0)
                {
                    tempArr[count] = gameBoard[i][j];
                    count++;
                }
            }
            gameBoard[i] = tempArr;
    ...
    public void mergeLeft() {
        for(int i = 0; i<gameBoard.length; i++)
        {
            for(int j=0; j<gameBoard[0].length-1; j++)
            {
                ...
                {
                    int count = j+1; 
                    while(gameBoard[i][j] == 0 && count < 4)
                    {
                        count++;
                    }
                    if(gameBoard[i][j]==gameBoard[i][count])
                    {
                        gameBoard[i][j]*=2;
                        gameBoard[i][count] = 0;
                    }
    ...
    public void flipRows() {
        int[][] tempArr = new int[gameBoard.length][gameBoard[0].length];
        for(int i=0; i<gameBoard.length; i++)
        {
            for(int j=0; j<gameBoard[0].length; j++)
            {
                tempArr[i][j] = gameBoard[i][gameBoard[0].length-j-1];
            }
        }
        gameBoard = tempArr;
    }
